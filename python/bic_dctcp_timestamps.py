...
class BIC_DCTCP_Flow():
    ...
    def __init__(self, datapath, datapath_info, cwnd_max, alpha_thres, backlog, preprocess):
        ...
        self.data_age = np.empty(self.backlog) * np.nan
        self.backlog_age_limit_seconds = 2 # time after which entries in the backlog (and the centroids) expire
        self.centroids = ()
        self.counter = 0
        self.n_valid = 0
        self.rtt_max = 0
    ...
    def on_report(self, r):
        if r.loss > 0 or r.sacked > 0:
            ...
            time_now = time.time()
            expired = (time_now - self.data_age) > self.backlog_age_limit_seconds
            self.data[expired] = np.nan
            if expired.sum() == self.n_valid:
                # if all the entries have expired, then reset the
                # centroids and start from scratch
                log.info('CENTROID RESET')
                self.counter = 0
                self.n_valid = 0
                self.rtt_max = 0
                self.rtt_min = np.inf
            ...
            # enter the new incoming RTT into the backlog
            self.data[self.counter] = rtt_feature
            self.data_age[self.counter] = time_now
            self.counter = (self.counter+1) % self.backlog
            self.n_valid += 1

            rtt_vecn = 0

            if self.centroid_mode == False:
                if self.n_valid == self.backlog:
                    # "centroid mode" of operation kicks in when the
                    # backlog becomes full with valid entries, and now
                    # the centroids are initialized 
                    self.centroid_mode = True
                    self.centroids = (self.data.min(), 2*self.data.max())
            else:
